...
export default function SmsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const result = await deleteSmsAction(id);
      if (result.success) {
                  ...
                  : messages.map((sms) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            disabled={deletingId === sms.id}
                            className="p-2 hover:bg-red-50 hover:text-red-600 rounded-lg text-gray-400 transition-colors disabled:opacity-50"
                            onClick={() => handleDelete(sms.id)} // Pass ID directly
                          >
                            {deletingId === sms.id ? (
